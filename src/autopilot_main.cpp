...
int main(int argc, char *argv[])
{
        ...
        double ele_trim = -0.0832; // rad
        double ail_trim = 0.0; // rad
        double rud_trim = 0.0; // rad
        double thr_trim = 0.49188; // percent throttle
        ...
        while (!handlerObject.stat.should_exit)
        {
				...
				if(!trim_values_set)
				{
					...
					// Convert PWM trim commands to rad and percent throttle
					ele_trim = 0.01745*((-ele_PWM_consts[1]+sqrt(ele_PWM_consts[1]*ele_PWM_consts[1]-4.0*ele_PWM_consts[0]*(ele_PWM_consts[2]-ele_trim_PWM)))/(2.0*ele_PWM_consts[0])); // rad
					ail_trim = 0.01745*((ail_trim_PWM - ail_PWM_consts[1])/ail_PWM_consts[0]); // rad
					rud_trim = 0.01745*((rud_trim_PWM - rud_PWM_consts[1])/rud_PWM_consts[0]); // rad
					thr_trim = ((thr_trim_PWM-(double)thr_PWM_min)/((double)thr_PWM_max-(double)thr_PWM_min)); // percent throttle
				}	
				...
				else if(handlerObject.rc_in.rc_chan[ap_engage_chan]<ap_engage_cutoff && trim_values_set)
				{
					...
					AoA = 0.01745 * eval_2D_poly(AoA_consts, 15, coeff_AoA, coeff_bet);    // in rad
					bet = 0.01745 * eval_2D_poly(bet_consts, 15, coeff_AoA, coeff_bet);    // in rad
					...
					{
							vel = sqrt((2.0 * (abs(total_pressure) - abs(static_pressure))) / (rho)); // in m/s
					}
                ...
                zcm.publish("ACTUATORS", &acts);
					...
					logfile_ap_test << get_gps_time(&handlerObject) << " " << curr_test_number << " " << vel << " " << AoA << " " << wyy << " " << pit << " " << bet << " " << wxx << " " << wzz << " " << rol << " " << yaw << std::endl;
					
					// If the initial states are already set and a test is running, update the states based on the linear dynamics of the system
					if(trim_values_set && initial_state_set && curr_test_number<=num_tests)
					{
						step_states( A, B, &states, inputs, 0.01);
