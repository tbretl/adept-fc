...
int main(int argc, char *argv[])
{
   ...
   float k_lon[2][4] = {  {0, 0, 0, 0},  {0, 0, 0, 0} }; // Longitudinal controller gains of form u_lon = -k_lon * x_lon
   float k_lat[2][5] = {  {0, 0.14855, 0, 0.4, 0},  {0, 0, 0, 0, 0} }; // Lateral controller gains of form u_lat = -k_lat * x_lat
   ...
   while (!handlerObject.stat.should_exit)
   {
       ...
       // INS data
       yaw = 0.01745329251 * objectHandler.vnins.yaw; // in rad
       pitch = 0.01745329251 * objectHandler.vnins.pitch; // in rad
       roll = 0.01745329251 * objectHandler.vnins.roll; // in rad
       wx = objectHandler.vnins.wx; // in rad/s (roll rate)
       wy = objectHandler.vnins.wy; // in rad/s (pitch rate)
       wz = objectHandler.vnins.wz; // in rad/s (yaw rate)
